...
# G1 = meshcuboid(width, height, height, h)
# G2 = -Mesh([point(-x,y,z) for (x,y,z) in vertices(G1)], deepcopy(cells(G1)))
...
for h in hs
    ...
    g1 = meshsegment(width, width, 3)
    g2 = CompScienceMeshes.translate(g1, point(0,0,height))
    g3 = Mesh([
            point(0,0,0),
            point(0,0,height)],
        [
            index(1,2)])
    g4 = CompScienceMeshes.translate(g3, point(width,0,0))
    g = CompScienceMeshes.weld(g1, g2, g3, g4)
    on_junction = overlap_gpredicate(g)
