...
# function runsim(;h, κ)
    ...
    Λ12 = Matrix(connectivity(G12, edges12, sign))
    Λ23 = Matrix(connectivity(Ĝ23, edges23, sign))
    ...
    Syy = assemble(@discretise(SL[p,q], p∈Y, q∈Y), materialize=mat)
    WSxx = assemble(@discretise(WS[p,q], p∈Y, q∈Y), materialize=mat)
    # HSxx = assemble(@discretise(HS[p,q], p∈X, q∈X), materialize=mat)
    # Syy = assemble(diagSkj; materialize=mat)
    pcd = 𝕄LHΣ * Sxx * MRΣ + 𝕄LHΣ * WSxx * MRΣ + 𝕄LHΣ * WSxx * MRΛH + 𝕄LHΣ * WSxx * MRΛH
    ...
    # v2, ch2 = solve(BEAST.GMRESSolver(P*sys, tol=2e-5, restart=250), P*rhs)
